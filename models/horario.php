    ...
    public function obtenerProfesoresGuardia()
{
    ...
    public function obtenerProfesoresAusentesPorDiaYHora($dia, $hora) {
        $query = "SELECT p.nom, p.cognoms, h.aula FROM ausencias a
    ...
    public function obtenerAulaProfesorAusente($id_profesor, $hora, $dia) {
        $query = "SELECT aula FROM horaris WHERE id_profesor = :id_profesor AND codi_hora = :hora AND dia = :dia";
        $stmt = $this->db->prepare($query);
    ...
    public function guardarAbsencia($diaId, $hora, $professorId) {
        try {
            // Comprovem si l'absencia existeix o no a la BDD
            ...
            if ($count > 0) {
                // Si existeix, donem un error
                return false;
            } else {
                // Si no existeix, inserim la absencia a la BDD
